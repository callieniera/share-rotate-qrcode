...
function parseExpiry(raw) {
   ...
   const s = String(raw).replace(/[^0-9:\.\-]/g, ' ').replace(/\s+/g, ' ').trim();
   const relative = String(raw).match(/(?:expire|expires|expiration)\s+(?:in\s+)?(\d+(?:\.\d+)?)\s*(?:seconds?|secs?|s)\b/i);
   if (relative) return Date.now() + Number(relative[1]) * 1000;
   if (s === '') return null;
   ...
   getOcrWorker()
      .then((worker) => worker.recognize(region, { user_patterns: cfg.ocrHint || 'expire in n secconds' }))
       .then(({ data }) => {
          ...
          log('OCR: "' + raw + '" -> ' + (parsed ? new Date(parsed).toISOString() : 'unparsed'));
          // Only apply if this is still the current rotation.
          if (raw && parsed && lastValue === current.value) {
             current.expiresAt = parsed;
...
function bindControls() {
   ...
   const oh = $('ocrHint');
   oh.value = cfg.ocrHint || 'expire in n secconds';
   oh.oninput = () => {
